...
function fit(){
 ...
 const load=document.getElementById('pfp-web-open-data');if(load){set(load,'height',compact?'29px':'30px');set(load,'min-height',compact?'29px':'30px');set(load,'padding','2px 12px');set(load,'font-size',compact?'11.5px':'12px');set(load,'font-weight','800');set(load,'letter-spacing','.45px');set(load,'color','#ffe17b');}
 const audio=document.getElementById('pfp-audio-hub-launch');if(audio){set(audio,'height',compact?'49px':'51px');set(audio,'min-height',compact?'49px':'51px');set(audio,'max-height',compact?'49px':'51px');set(audio,'padding',compact?'4px 14px':'5px 14px');const am=audio.querySelector('.pfp-audio-main');const as=audio.querySelector('.pfp-audio-sub');if(am)set(am,'font-size',compact?'16px':'17px');if(as)set(as,'font-size',compact?'6.4px':'6.8px');}
