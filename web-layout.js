...
function fit(){
 ...
 const load=document.getElementById('pfp-web-open-data');if(load){set(load,'height',compact?'37px':'39px');set(load,'min-height',compact?'37px':'39px');set(load,'padding','3px 14px');set(load,'font-size',compact?'13.5px':'14px');set(load,'font-weight','900');set(load,'letter-spacing','.55px');set(load,'color','#ffe17b');}
 const audio=document.getElementById('pfp-audio-hub-launch');if(audio){set(audio,'height',compact?'41px':'42px');set(audio,'min-height',compact?'41px':'42px');set(audio,'max-height',compact?'41px':'42px');set(audio,'padding',compact?'3px 14px':'3px 14px');const am=audio.querySelector('.pfp-audio-main');const as=audio.querySelector('.pfp-audio-sub');if(am)set(am,'font-size',compact?'14.5px':'15px');if(as)set(as,'font-size',compact?'5.8px':'6px');}
