...
let CURRENT_THEME: GameTheme = THEMES.sfa;

export function setCurrentTheme(themeId: 'sfa' | 'dp'): void {
...
// Keep names the same, but make them dynamic functions.
export function HIGHLIGHT_COLOR(): string { return CURRENT_THEME.highlight; }
export function COOL_BLUE_COLOR(): string { return CURRENT_THEME.coolBlue; }
export function PANEL_BG_COLOR(): string { return CURRENT_THEME.panelBg; }
...
export class TextEntry implements Widget {
    setRange(arg0: number, arg1: number) {
...
flair.bulletColor = !!this.itemIsOn[i] ? HIGHLIGHT_COLOR() : '#aaa';            flair.color = !!this.itemIsOn[i] ? 'white' : '#aaa';
        }
        this.setInternalFlairs(flairs);
    }
...
export class Panel implements Widget {
    ...
    private syncSize() {
    const widthExpanded = this.expanded;
...
this.svgIcon.style.fill = this.expanded ? '#1A1258' : '';
...
private searchEntry!: TextEntry;
    ...
    constructor(public viewer: Viewer.Viewer) {
        ...
        // --- Game switch row (SELECT GAME label only; NO mini logos) ---
// --- Game switch row (left header). Mini logos hidden on landing ---
this.gameRow = document.createElement('div');
this.gameRow.style.display = 'flex';
this.gameRow.style.alignItems = 'center';
this.gameRow.style.justifyContent = 'space-between';
this.gameRow.style.gap = '12px';
this.gameRow.style.padding = '10px 12px';
this.gameRow.style.background = 'rgba(0, 0, 0, 1.0)';
this.gameRow.style.borderBottom = '1px solid rgba(255,255,255,0.06)';

const leftLabel = document.createElement('div');
leftLabel.textContent = 'SELECT GAME';
leftLabel.style.fontWeight = 'bold';
leftLabel.style.letterSpacing = '1px';
leftLabel.style.color = 'white';
leftLabel.style.userSelect = 'none';

this.gameMiniIcons = document.createElement('div');
this.gameMiniIcons.style.display = 'none'; // <-- HIDE ON LANDING
this.gameMiniIcons.style.display = 'none';
this.gameMiniIcons.style.flexDirection = 'row';
this.gameMiniIcons.style.gap = '10px';
this.gameMiniIcons.style.alignItems = 'center';
this.gameMiniIcons.style.justifyContent = 'end';
this.gameMiniIcons.style.pointerEvents = 'auto';

const makeMini = (src: string, title: string, gameId: 'sfa' | 'dp') => {
...
img.onclick = (e) => {
    e.stopPropagation();
    this.onGameLogoClicked(gameId);
};
    return img;
};

this.gameMiniIcons.appendChild(makeMini(sfaLogoURL, 'Star Fox Adventures', 'sfa'));
this.gameMiniIcons.appendChild(makeMini(dpLogoURL, 'Dinosaur Planet', 'dp'));

this.gameRow.appendChild(leftLabel);
...
const musicCheckbox = new Checkbox('Disable Music', false);

musicCheckbox.onchanged = () => {
    const musicState = (window as any).musicState;

    musicState.muted = musicCheckbox.checked;

    if (musicState.muted && musicState.audio) {
...
this.contents.appendChild(this.landingBlurb);
        ...
        this.topBox.style.background = 'rgba(0, 0, 0, 1.0)';
        this.contents.append(this.topBox);
...
if (sceneDescScroll) {
    sceneDescScroll.classList.add('sfa-list-scroll');
        ...
        this.gameLanding.style.backgroundColor = 'rgba(0, 0, 0, 0.8)';
        this.gameLanding.style.display = 'grid';
        this.gameLanding.style.alignContent = 'start';
        this.gameLanding.style.padding = '18px';
        this.gameLanding.style.gap = '16px';
        this.gameLanding.style.borderRadius = '16px';
        this.gameLanding.style.border = '1px solid rgba(43,43,184,.26)';
        this.gameLanding.style.boxShadow = '0 18px 48px rgba(0,0,0,.6), inset 0 0 1px rgba(255,255,255,.04)';

        const landingTitle = document.createElement('div');
        landingTitle.textContent = 'SELECT GAME';
        landingTitle.style.font = 'bold 20px monospace';
        landingTitle.style.letterSpacing = '2px';
        landingTitle.style.color = 'white';
        landingTitle.style.userSelect = 'none';
        this.gameLanding.appendChild(landingTitle);

        const landingRow = document.createElement('div');
        landingRow.style.display = 'grid';
        landingRow.style.gridTemplateColumns = '1fr';
        landingRow.style.gap = '14px';
        this.gameLanding.appendChild(landingRow);

        const makeBigLogo = (src: string, title: string, onClick: () => void): HTMLElement => {
            ...
            btn.style.background = 'linear-gradient(180deg, rgba(17,37,64,1), rgba(11,24,44,1))';
            btn.style.border = '1px solid rgba(43,43,184,.26)';
            btn.style.boxShadow = '0 14px 28px rgba(0,0,0,.55), inset 0 0 1px rgba(255,255,255,.05)';
            btn.style.transition = 'transform 0.12s ease, box-shadow 0.12s ease';

            const img = document.createElement('img');
            img.src = src;
            img.style.width = '220px';                 // BIG LOGO SIZE
            ...
            btn.onmouseenter = () => {
                ...
                btn.style.boxShadow = '0 18px 40px rgba(0,0,0,.65), inset 0 0 1px rgba(255,255,255,.06)';
                btn.style.borderColor = 'rgba(224,181,78,.55)';
            };
            btn.onmouseleave = () => {
                btn.style.transform = 'scale(1)';
                btn.style.boxShadow = '0 14px 28px rgba(0,0,0,.55), inset 0 0 1px rgba(255,255,255,.05)';
                btn.style.borderColor = 'rgba(43,43,184,.26)';
            };
        ...
        landingRow.appendChild(makeBigLogo(
            sfaLogoURL,
            'Star Fox Adventures',
() => this.onGameLogoClicked('sfa'),
        ));

        landingRow.appendChild(makeBigLogo(
            dpLogoURL,
            'Dinosaur Planet',
() => this.onGameLogoClicked('dp'),     ));

        this.extraRack.appendChild(this.gameLanding);
        this.extraRack.appendChild(this.sceneDescList.elem);

        // Start on landing
        this.showGameLanding(true);
 this.applyTheme();
...
public showGameLanding(v: boolean): void {
    // RIGHT side
    this.gameLanding.style.display = v ? 'grid' : 'none';
    (this.sceneDescList.elem as HTMLElement).style.display = v ? 'none' : 'block';

    // LEFT side (hide lists/search/random while landing)
...
.Slider::-moz-range-thumb {
    ...
    height: 24px;
    cursor: pointer;
background: ${HIGHLIGHT_COLOR()};    border-radius: 0;
    border: none;
}
...
this.icon.style.color = HIGHLIGHT_COLOR();            this.tooltipElem.style.opacity = '0';
...
class ShareButton extends PanelButton {
    ...
    private setCopyButtonState(state: 'copy' | 'copied'): void {
        ...
        if (state === 'copy') {
this.copyButton.style.backgroundColor = HIGHLIGHT_COLOR();            this.copyButton.style.cursor = 'pointer';
...
export class UI {
    public elem: HTMLElement;
public forcePanelsMouseOutState(): void {
    // Match panelToplevel.onmouseout behavior
    this.panelToplevel.style.backgroundColor = 'rgba(0, 0, 0, 0)';
    this.panelToplevel.style.overflow = 'hidden';

    // Optional: auto-close panels
    if (this.shouldPanelsAutoClose())
        this.setPanelsAutoClosed(true);
}
public applyGameTheme(gameId: 'sfa' | 'dp'): void {
    setCurrentTheme(gameId);

    // Re-theme known panel accents that cache colors
    if (this.sceneSelect)
        this.sceneSelect.applyTheme();

    // Layer panels use a fixed custom header color at construction time
    for (const panel of this.panels ?? []) {
        if (panel instanceof LayerPanel) {
            panel.customHeaderBackgroundColor = COOL_BLUE_COLOR();
        }
        panel.syncExpanded(); // refresh header style
    }

    // Refresh UI widgets that use accent color dynamically
    this.playPauseButton.syncStyle();
    this.shareButton.syncStyle();
    this.fullscreenButton.syncStyle();

    const speed = this.viewer.cameraController?.getKeyMoveSpeed?.() ?? 1;
    this.cameraSpeedIndicator.setCameraSpeed(speed, false);
    ...
    public sceneChanged() {
        if (!this.viewer.cameraController) return;
