...
export class DataFetcher {
    ...
    private completedCache = new Map<string, NamedArrayBufferSlice>();
public debug = false;
...
if (this.completedCache.has(url))
    return Promise.resolve(this.completedCache.get(url)!);
    if (this.aborted)
    ...
    this.completedCache.set(url, result);
});
