...
// --- N64 BIN DECODER ---
function decodeRareN64Texture(data: DataView): { width: number, height: number, cms: number, cmt: number, pixels: Uint8Array } | null {
    if (data.byteLength < 0x20) return null;
    ...
    return { width, height, cms, cmt, pixels: rgba8 };
...
export class SFATextureFetcher extends TextureFetcher {
    ...
    public static async create(gameInfo: GameInfo, dataFetcher: DataFetcher, isBeta: boolean): Promise<SFATextureFetcher> {
        ...
        try {
            ...
            self.texpre = texpre;
        } catch (e) { self.textableBin = new DataView(new ArrayBuffer(0x8000)); self.texpre = null; }
    ...
    public destroy(device: GfxDevice) {
        ...
        this.subdirTextureFiles = {}; this.fakes.destroy(device);
    }
}