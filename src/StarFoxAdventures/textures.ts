...
const SFA_PALM_FROND_CLAMP_S_TEXIDS = new Set<number>([
]);

const SFA_PALM_FROND_CLAMP_T_TEXIDS = new Set<number>([
1680]);

function overrideTextureArraySampler(
    cache: GfxRenderCache,
    texArray: SFATextureArray,
    wrapS: GfxWrapMode | null,
    wrapT: GfxWrapMode | null,
): void {
    for (const tex of texArray.textures) {
        const prev = (tex as any).__exportSampler as ExportSamplerInfo | undefined;

        const samplerInfo: ExportSamplerInfo = {
            wrapS: wrapS ?? prev?.wrapS ?? GfxWrapMode.Repeat,
            wrapT: wrapT ?? prev?.wrapT ?? GfxWrapMode.Repeat,
            minFilter: prev?.minFilter ?? GfxTexFilterMode.Bilinear,
            magFilter: prev?.magFilter ?? GfxTexFilterMode.Bilinear,
            mipFilter: prev?.mipFilter ?? GfxMipFilterMode.Nearest,
        };
        ...
        tex.updateTextureAndNotify(tex.gfxTexture, tex.width, tex.height, gfxSampler);
...
if (this.textures[num] === undefined) {
    try {
        ...
        if (texture !== null) {
            if (SFA_PALM_FROND_CLAMP_S_TEXIDS.has(num))
                overrideTextureArraySampler(cache, texture, GfxWrapMode.Clamp, null);

            if (SFA_PALM_FROND_CLAMP_T_TEXIDS.has(num))
                overrideTextureArraySampler(cache, texture, null, GfxWrapMode.Clamp);

            for (let i = 0; i < texture.textures.length; i++) {
                if (texture.textures[i].viewerTexture)
                    texture.textures[i].viewerTexture!.name = `${this.name} #${num}${texture.textures.length > 1 ? '.' + i : ''}`;
            }
        }
...
async function fetchTextureFile(
    dataFetcher: DataFetcher,
    tabPath: string,
    binPath: string,
    isBeta: boolean,
): Promise<TextureFile | null> {
    const fetchOptional = async (path: string): Promise<ArrayBufferSlice | null> => {
        try {
    ...
    const [tab, bin] = await Promise.all([
        fetchOptional(tabPath),
        fetchOptional(binPath),
    ]);
...
export function debugResolveEarly3TextureId(texId: number, modelId: number = 0): number | null {
    void modelId;

    if (early3TextureIdMap[texId] !== undefined)
        return early3TextureIdMap[texId];
...
export function debugResolveEarly4TextureId(texId: number, modelId: number = 0): number | null {
 const perModel = EARLY4_PER_MODEL_REMAP[modelId | 0];

 if (perModel && perModel[texId] !== undefined)
  return perModel[texId];

 if (early4TextureIdMap[texId] !== undefined)
  return early4TextureIdMap[texId];

 return null;
}

export function debugResolveAncientTextureId(texId: number, modelId: number = 0): number | null {
    const perModel = ANCIENT_DP_PER_MODEL_REMAP[modelId | 0];
    const perModelHit = perModel ? perModel[texId] : undefined;

    if (perModelHit !== undefined)
        return perModelHit.id !== 5555 ? perModelHit.id : null;

    if (ancientMapTextureIdMap[texId] !== undefined) {
        const mapped = ancientMapTextureIdMap[texId];
        return mapped !== 5555 ? mapped : null;
    }
...
const DP_FORCE_MIRROR_S_TEXIDS = new Set<number>([
 1172,1173,1026,1027,571,559,1066 // put broken texId here
]);

const DP_FORCE_MIRROR_T_TEXIDS = new Set<number>([
]);
export class SFATextureFetcher extends TextureFetcher {
private isDPTextureMode(): boolean {
    return (
        this.modelVersion === ModelVersion.DinosaurPlanet ||
        this.modelVersion === ModelVersion.Mod49Old
    );
}

private getDPPathBaseOverride(): string | undefined {
    return this.modelVersion === ModelVersion.Mod49Old
        ? DP_GAME_INFO.pathBase
        : undefined;
}
...
if (this.modelVersion === ModelVersion.Mod49Old)
    return {
        texId,
        useDPBin: true,
...
const [textableBin, texpre, rootTex0, rootTex1] = await Promise.all([
    dataFetcher.fetchData(`${pathBase}/TEXTABLE.bin`, { allow404: true }).catch(() => null),
