...
export class XRCameraController {
    ...
    // Teleport State
    public aimingTeleport: boolean = false;
    public teleportTarget = vec3.create();
    private rightStickReleased: boolean = true;
    private rightStickXReady: boolean = true;
...
const mainCamera = this.leftCamera;
        ...
        for (const source of inputSources) {
            ...
            // --- Height Triggers (Speed increased here) ---
            if (gamepad.buttons.length >= 2) {
                const upDown = (gamepad.buttons[0].value - gamepad.buttons[1].value);
                if (Math.abs(upDown) > 0.01) {
                    // CHANGED FROM 0.05 TO 0.20 FOR FASTER MOVEMENT
                    this.offset[1] += upDown * 0.20 * this.worldScale;
                }
            }
            ...
            if (gamepad.axes.length >= 4) {
                ...
                // RIGHT STICK: Snap Turn & Teleport
                else if (source.handedness === 'right') {
                    const stickX = gamepad.axes[2];
                    const stickY = gamepad.axes[3];

                    // Snap Turn
                    if (Math.abs(stickX) > 0.5 && !this.aimingTeleport) {
                        if (this.rightStickXReady) {
                            this.rotationOffset -= Math.sign(stickX) * (Math.PI / 4);
                            this.rightStickXReady = false;
                        }
                    } else if (Math.abs(stickX) < 0.2) {
                        this.rightStickXReady = true;
                    }

                    // Teleport
                    if (stickY < -0.5) {
                        this.aimingTeleport = true;
                        this.showTeleportRing = true;
                        this.rightStickReleased = false;

                        const jumpDist = 15.0 * this.worldScale;
                        
                        this.teleportTarget[0] = this.offset[0] + (camForward[0] * jumpDist);
                        this.teleportTarget[1] = this.offset[1];
                        this.teleportTarget[2] = this.offset[2] + (camForward[2] * jumpDist);
                        
                        vec3.copy(this.ringPosition, this.teleportTarget);
                    } 
                    else if (this.aimingTeleport && stickY > -0.2) {
                        vec3.copy(this.offset, this.teleportTarget);
                        this.aimingTeleport = false;
                        this.showTeleportRing = false;
                        this.rightStickReleased = true;
                    }
                }
            }
        }
        ...
        if (Math.abs(moveX) > 0.1 || Math.abs(moveZ) > 0.1) {
            const moveSpeed = 0.5 * this.worldScale;

            this.offset[0] += camRight[0] * moveX * moveSpeed;
            this.offset[2] += camRight[2] * moveX * moveSpeed;

            this.offset[0] += camForward[0] * (-moveZ) * moveSpeed; 
            this.offset[2] += camForward[2] * (-moveZ) * moveSpeed;
        }
