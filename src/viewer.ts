...
if (!(updateInfo.webXRContext !== null &&
      updateInfo.webXRContext.views &&
      updateInfo.webXRContext.xrSession)) {
    camera.setClipPlanes(0.01);
}
//test
...
if (!(this as any).__xrSynced) {
            // Sync Position
            this.xrCameraController.offset[0] = this.camera.worldMatrix[12];
...
this.xrCameraController.rotationOffset = Math.atan2(-fwdX, -fwdZ);
    ...
    // Reset sync flag when leaving XR
    (this as any).__xrSynced = false;
