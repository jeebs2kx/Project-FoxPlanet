...
function alias(m){if(!m||!(m.entries instanceof Map))return;for(const [k0,e] of [...m.entries.entries()]){const k=clean(k0).toLowerCase();let x=k.match(/^(starfoxadventuresdemo|starfoxadventures)\/files\/(.+)$/i);if(x&&!m.entries.has((x[1]+'/'+x[2]).toLowerCase()))m.entries.set((x[1]+'/'+x[2]).toLowerCase(),e);x=k.match(/(?:^|\/)gamedata\/(starfoxadventuresdemo|starfoxadventures|dinosaurplanet|dinosaurplanet_vanilla|sequence-data)\/(.+)$/i);if(x&&!m.entries.has((x[1]+'/'+x[2]).toLowerCase()))m.entries.set((x[1]+'/'+x[2]).toLowerCase(),e);}}
...
install();window.addEventListener('load',()=>{install();setTimeout(install,250);});window.addEventListener('hashchange',route,true);window.addEventListener('popstate',route,true);window.addEventListener('pfp-local-data-mounted',()=>{remember();setTimeout(install,30);});setInterval(watch,500);
